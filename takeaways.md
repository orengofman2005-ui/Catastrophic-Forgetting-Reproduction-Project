...
בסעיף זה אנו משווים את הגרפים שהפקנו לגרפי המאמר המקורי של Goodfellow et al. (2015), תרחיש אחר תרחיש. ההשוואה מתמקדת בשני סוגי גרפים: גרפי ה-Pareto frontier (tradeoff בין המשימות) וגרפי גדלי המודל האופטימלי.
...
**גדלי המודל האופטימלי — תרחיש 1:**

![Scenario 1 Model Sizes](results_fixed/fig_s1_params.png)

גרף גדלי המודל מאשר את ממצא המאמר (Figure 2): Dropout דורש רשת גדולה יותר מ-SGD בכל פונקציות האקטיבציה, במיוחד ב-ReLU ו-LWTA. הסקאלה אצלנו מגיעה עד ~7M פרמטרים לעומת ~50M במאמר, הבדל המוסבר על ידי hidden_dim search space קטן יותר (עד 2048 לעומת ערכים גדולים יותר במאמר).
...
**גדלי המודל האופטימלי — תרחיש 2:**

![Scenario 2 Model Sizes](results_fixed/fig_s2_params.png)

גרף גדלי המודל מאשר את ממצא המאמר (Figure 4): ב-Sigmoid וב-LWTA Dropout דורש רשת גדולה משמעותית יותר מ-SGD, בעוד ב-Maxout הגדלים דומים — בדיוק כפי שמוצג במאמר המקורי. זהו הממצא הכי עקבי בין הגרפים שלנו למאמר.
...
**גדלי המודל האופטימלי — תרחיש 3:**

![Scenario 3 Model Sizes](results_fixed/fig_s3_params.png)

בתרחיש המשימות השונות, המאמר מציין שבניגוד לתרחישים האחרים, Dropout דווקא **מקטין** את גודל המודל האופטימלי בחלק מהמקרים. ממצא זה שוחזר חלקית בגרף שלנו — ב-Maxout וב-ReLU הגדלים דומים בין SGD ל-Dropout, בעוד ב-LWTA ניכרת עדיין עליה קלה עם Dropout.

---

## סיכום ההשוואה

| תרחיש | מגמת Frontier | שוחזרה? | מגמת גדלי מודל | שוחזרה? |
|--------|--------------|---------|----------------|---------|
| 1 — MNIST Permutation | Dropout בחזית, מעטפת יורדת | ✅ כן | Dropout → מודל גדול יותר | ✅ כן |
| 2 — Amazon Similar | צורת U, Dropout בחזית | ✅ כן | Dropout → גדול ב-Sigmoid/LWTA, שווה ב-Maxout | ✅ כן |
| 3 — MNIST+Amazon Dissimilar | שגיאת old task נמוכה מאוד | ✅ כן | Dropout → גודל דומה או קטן יותר | ✅ חלקית |

**מסקנה כללית:** כל הממצאים המרכזיים של המאמר — עליונות Dropout, תלות צורת המעטפת בסוג המשימות, שגיאת old task נמוכה במשימות שונות, וקשר Dropout לגודל המודל האופטימלי — שוחזרו בהצלחה ברמה האיכותית. ההבדלים הכמותיים שנצפו מוסברים על ידי גורמים ידועים: מספר trials מוגבל, הבדלי עיבוד נתונים ב-Amazon, ומנגנון early stopping שמנע שכחה קיצונית.
